...
function skill_selection.init(ownerRef)
	owner = ownerRef
...
function skill_selection.getWeightedRandomSkillId(availableSkills)
	...
	if skill_selection.localRandomCount == nil then
		...
		if owner.PLUS_DEBUG then LOG("PLUS Ext: Initialized RNG with seed " .. seed .. " and fast-forwarded " .. savedCount .. " times") end
	end
...
function skill_selection.applySkillsToPilot(pilot)
	...
	if storedSkills ~= nil then
		if owner.PLUS_DEBUG then LOG("PLUS Ext: Read stored skill") end
	-- if its the time traveler, save the current skills
	elseif owner.timeTraveler and owner.timeTraveler._address == pilot._address then
		local lus = owner.timeTraveler:getLvlUpSkills()
		storedSkills = {lus:getSkill1():getIdStr(), lus:getSkill2():getIdStr()}
		GAME.cplus_plus_ex.pilotSkills[pilotId] = storedSkills
		if owner.PLUS_DEBUG then LOG("PLUS Ext: Read time traveler skills") end
	-- otherwise assign random skills
		...
		if owner.PLUS_DEBUG then LOG("PLUS Ext: Assigning random skills")
		end
	end
...
function skill_selection.applySkillsToAllPilots()
	...
	if #skill_config_module.enabledSkillsIds == 0 then
		if owner.PLUS_DEBUG then LOG("PLUS Ext: No enabled skills, skipping pilot skill assignment") end
		return
	end
...
function skill_selection.markPerRunSkillAsUsed(skillId)
	...
	if skill_config_module.config.skillConfigs[skillId].reusability == owner.REUSABLILITY.PER_RUN then
